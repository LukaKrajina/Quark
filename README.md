...
`int8` `int16` `int32` `int64` `uint8` `uint16` `uint32` `uint64` `float` `double` `string` `char` `Qubit` `QObject` `QModel`（另有 `auto` / `let` 类型推导、`cap<T>` 能力指针、`fixed` 编译期常量、`null` 空值）
...
qk 支持裸机 / 内核编程：`cap<T>` 能力指针、`unsafe { ... }` 危险操作块、`native("hlt")` 内联汇编、`outb`/`inb` 端口 I/O、`sync_*` 原子操作、`qk_gc_alloc`/`qk_gc_free` 内核堆、`addr` 取函数地址、`qk_sys_call`/`qk_sys_log` 系统调用 ABI 与 `qk_qms_gap`/`qk_mix_bound`/`qk_qms_conc` QMS 数值内核。
