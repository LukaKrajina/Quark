...
class SemanticAnalyzer {
    constructor() {
        ...
        this.fixedSymbols = new Set();
        this.flavorMembers = new Map();
        this.externSigs = new Map();
    }
    resetDeclarations() {
        this.forms.clear();
        this.traits.clear();
    ...
    checkInheritance() {
        for (const [name, form] of this.forms) {
            if (form.inherits)
                this.checkSingleInherit(name, form.inherits.base, form.inherits.ranks, 'form', form);
        }
        for (const [name, trait] of this.traits) {
            if (trait.inherits)
                this.checkSingleInherit(name, trait.inherits.base, trait.inherits.ranks, 'trait', trait);
        }
    }
    checkSingleInherit(child, parent, ranks, kind, decl) {
     ...
     */
    runBorrowCheck(program) {
        let mir;
        try {
            mir = (0, mir_1.buildMir)(program);
        }
        catch (err) {
        ...
        for (const body of mir.bodies) {
            for (const e of new borrow_1.BorrowChecker(body).check()) {
                this.errors.push({
     ...
     */
    runRaceDetection(program) {
        // 遍历所有函数体（不再仅限 quark_main），对每个函数体独立做
        // spawn/entangle/sync_*/measure 的 digest 分析；tid 用函数名区分。
        const functions = program.body.filter(s => s.type === 'FunctionDeclaration' && Array.isArray(s.body));
        if (functions.length === 0)
            return;
        const classic = [];
        const quantum = [];
        const entanglePairs = [];
        const threadQubits = new Map();
        let tidCounter = 0;
        const touchQubit = (tid, q) => {
            if (q === 'mem')
                return;
            if (!threadQubits.has(tid))
                threadQubits.set(tid, new Set());
            threadQubits.get(tid).add(q);
        };
        const varName = (e) => (e && e.type === 'Identifier' ? e.name : 'mem');
        ...
        const walkExpr = (expr, tid) => {
            ...
            switch (expr.type) {
                case 'FunctionCall': {
                    ...
                    expr.arguments.forEach((a) => walkExpr(a, tid));
                    return;
                }
                case 'BinaryExpression':
                    walkExpr(expr.left, tid);
                    walkExpr(expr.right, tid);
                    return;
                ...
                case 'UnaryExpression':
                    walkExpr(expr.argument, tid);
                    return;
                case 'Dereference':
                    walkExpr(expr.target, tid);
                    return;
                case 'AddressOf':
                    walkExpr(expr.target, tid);
                    return;
                case 'MemberExpression':
                    walkExpr(expr.object, tid);
                    expr.arguments.forEach((a) => walkExpr(a, tid));
                    return;
                case 'NewExpression':
                    expr.arguments.forEach((a) => walkExpr(a, tid));
                ...
                case 'IndexExpression':
                    walkExpr(expr.object, tid);
                    expr.indices.forEach((a) => walkExpr(a, tid));
                    return;
                case 'FuseExpression':
                    walkExpr(expr.discriminant, tid);
                    expr.arms.forEach((arm) => { walkExpr(arm.pattern, tid); walkExpr(arm.value, tid); });
                    return;
                case 'FunctionExpression':
                    walkStmts(expr.body, tid);
                    return;
                default: return; // 字面量 / 标识符 / native / result 等叶子
            }
        };
        const walkStmts = (stmts, tid) => {
            for (const s of stmts) {
                if (!s)
                    continue;
                switch (s.type) {
                    case 'SpawnStatement':
                        walkStmts(s.body, 't' + (++tidCounter));
                        break;
                    case 'EntangleStatement': {
                        const l = varName(s.left), r = varName(s.right);
                        if (l !== 'mem' && r !== 'mem')
                            entanglePairs.push([l, r]);
                        touchQubit(tid, l);
                        touchQubit(tid, r);
                        walkExpr(s.left, tid);
                        walkExpr(s.right, tid);
                        break;
                    }
                    case 'VariableDeclaration':
                        walkExpr(s.value, tid);
                        break;
                    case 'ExpressionStatement':
                        walkExpr(s.expression, tid);
                        break;
                    case 'AssignmentStatement':
                        walkExpr(s.target, tid);
                        walkExpr(s.value, tid);
                        break;
                    case 'ReturnStatement':
                        walkExpr(s.argument, tid);
                        break;
                    case 'IfStatement':
                        walkExpr(s.condition, tid);
                        walkStmts(s.consequent, tid);
                        if (s.alternate)
                            walkStmts(s.alternate, tid);
                        break;
                    case 'WhileStatement':
                        walkExpr(s.condition, tid);
                        walkStmts(s.body, tid);
                        if (s.elseBody)
                            walkStmts(s.elseBody, tid);
                        break;
                    case 'ForStatement':
                        if (s.init)
                            walkStmts([s.init], tid);
                        if (s.condition)
                            walkExpr(s.condition, tid);
                        walkStmts(s.body, tid);
                        if (s.update)
                            walkStmts([s.update], tid);
                        break;
                    case 'RouteStatement':
                        walkExpr(s.discriminant, tid);
                        s.cases.forEach((c) => { walkExpr(c.value, tid); walkStmts(c.body, tid); });
                        if (s.fallback)
                            walkStmts(s.fallback, tid);
                        break;
                    case 'SpinStatement':
                        walkStmts(s.body, tid);
                        walkExpr(s.condition, tid);
        ...
        const closure = (0, race_1.entanglementClosure)(entanglePairs);
        const closureOf = (q) => {
        ...
        const threadClosure = new Map();
        for (const [tid, qs] of threadQubits) {
            const acc = new Set();
            for (const q of qs)
                closureOf(q).forEach(x => acc.add(x));
            threadClosure.set(tid, acc);
        }
        const makeDigest = (tid) => ({
            tid,
            locks: new Set(),
            qubits: threadClosure.get(tid) ?? new Set(),
        });
        const classicAccesses = classic.map(a => ({
            variable: a.variable,
            isWrite: a.isWrite,
            digest: makeDigest(a.tid),
            destructiveQubits: new Set(),
            line: a.line,
            column: a.column,
        }));
        const quantumAccesses = quantum.map(a => ({
            variable: a.variable,
            isWrite: a.isWrite,
            digest: makeDigest(a.tid),
        ...
        for (const r of (0, race_1.detectRaces)(classicAccesses)) {
            this.errors.push({
        ...
        for (const r of (0, race_1.detectQuantumRaces)(quantumAccesses)) {
            this.errors.push({
    ...
    checkQubitArgs(expr, expectedCount) {
    ...
    visitExpression(expr) {
        ...
        if (expr.type === 'FuseExpression') {
            ...
            for (const arm of expr.arms) {
                if (arm.pattern)
                    this.visitExpression(arm.pattern);
