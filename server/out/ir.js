    ...
    getOrCreateStringConstant(value) {
        ...
        for (let i = 0; i < utf8.length; i++) {
            ...
            if (b === 34)
                ...
                llvmStr += "\\" + b.toString(16).padStart(2, '0');
            else
                llvmStr += String.fromCharCode(b);
        }
    ...
    visitStatement(stmt) {
        ...
        if (stmt.type === 'VariableDeclaration') {
            ...
            if (stmt.target && stmt.target.type === 'IndexExpression') {
                ...
                const indices = stmt.target.indices;
    ...
    visitFunctionDeclaration(func) {
        ...
        if (func.attributes && func.attributes.length > 0) {
            const parts = func.attributes
                .map(a => {
                if (a.name === 'place' && a.value)
                    return `section "${a.value}"`;
                if (a.name === 'raw')
                    return 'naked';
                return '';
            })
