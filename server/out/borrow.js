...
function buildPredecessors(body) {
    const preds = body.blocks.map(() => []);
...
class BorrowChecker {
    ...
    checkLinearity() {
        ...
        const live = this.body.blocks.map(() => new Set());
        const consumed = this.body.blocks.map(() => new Set());
        // 初始：所有线性局部都"未消费"？不——局部变量在首次 QAlloc 赋值前并不存在。
        // 用前向数据流：QAlloc 把局部加入 live；消费将其移除。
        // 工作队列迭代到不动点。
        const work = new Set([0]);
        let iterations = 0;
        const maxIter = n * 4 + 16;
        while (work.size > 0 && iterations++ < maxIter) {
            const id = work.values().next().value;
            work.delete(id);
            const inLive = new Set();
            const inConsumed = new Set();
            if (id === 0) {
                // 入口：无线性局部存活
            }
            else {
                for (const p of preds[id]) {
                    for (const l of live[p])
                        inLive.add(l);
                    for (const l of consumed[p])
                        inConsumed.add(l);
                }
            }
            ...
            for (const stmt of this.body.blocks[id].statements) {
                for (const acc of statementAccess(stmt)) {
                    if (!this.isLinear(acc.place.local))
                        continue;
                    if (acc.kind === 'borrow') {
                    ...
                    if (curConsumed.has(acc.place.local)) {
                        this.err('E-Q001', `Quantum linearity violation: '${this.name(acc.place.local)}' was already consumed; Qubit/QObject cannot be cloned or measured twice.`, acc.place.local);
                    }
            ...
            if (term && term.kind === 'Return') {
                for (const l of curLive) {
                    this.err('E-Q002', `Quantum linearity violation: '${this.name(l)}' is allocated but never measured or released on this path.`, l);
                }
            }
            // 收敛判断：若 out 状态没变，跳过后继
            const outChanged = !sameSet(curLive, live[id]) || !sameSet(curConsumed, consumed[id]);
            live[id] = curLive;
    ...
    checkBorrows() {
        ...
        const loanByTemp = new Map();
        for (const l of loans)
            loanByTemp.set(l.temp, l);
    ...
    computeLiveness() {
        const n = this.body.blocks.length;
        const use = this.body.blocks.map(() => new Set());
        const def = this.body.blocks.map(() => new Set());
        for (const b of this.body.blocks) {
            const defSoFar = new Set();
        ...
        const liveIn = this.body.blocks.map(() => new Set());
        const liveOut = this.body.blocks.map(() => new Set());
        let changed = true;
        while (changed) {
            changed = false;
            for (const b of this.body.blocks) {
     ...
     */
    checkLoanConflicts(stmt, activeByPlace) {
        for (const [place, c] of activeByPlace) {
            if (c.mut >= 2) {
                this.err('E0499', `cannot borrow '${this.name(place)}' as mutable more than once at the same time.`, place);
            }
            if (c.mut >= 1 && c.shared >= 1) {
                this.err('E0499', `cannot borrow '${this.name(place)}' as mutable and immutable at the same time.`, place);
            }
        }
        for (const p of writesOf(stmt)) {
            if (activeByPlace.has(p)) {
                this.err('E0506', `cannot assign to '${this.name(p)}' while it is borrowed.`, p);
            }
        }
        ...
        for (const a of statementAccess(stmt)) {
            if (a.kind !== 'borrow' || isPersistentRef)
                continue;
            const p = rootLocal(a.place);
            const e = activeByPlace.get(p);
            if (e && (a.mut || e.mut >= 1)) {
                this.err('E0499', `cannot borrow '${this.name(p)}' while it is already borrowed.`, p);
            }
        }
    }
    /** 语句内冲突检查：同一语句内的多个可变借用 / mut+shared / 借用+写入 */
    checkIntraStatement(stmt) {
        ...
        for (const [r, c] of mutBorrows) {
            if (c > 1) {
                this.err('E0499', `cannot borrow '${this.name(r)}' as mutable more than once at the same time.`, r);
            }
            if ((sharedBorrows.get(r) ?? 0) > 0) {
                this.err('E0499', `cannot borrow '${this.name(r)}' as mutable and immutable at the same time.`, r);
            }
            if ((writes.get(r) ?? 0) > 0) {
                this.err('E0506', `cannot assign to '${this.name(r)}' while it is mutably borrowed.`, r);
            }
        }
        for (const [r] of sharedBorrows) {
            if ((writes.get(r) ?? 0) > 0) {
                this.err('E0506', `cannot assign to '${this.name(r)}' while it is borrowed as immutable.`, r);
            }
        }
    }
    isLinear(id) {
...
exports.BorrowChecker = BorrowChecker;
