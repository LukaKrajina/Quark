...
function main() {
    ...
    try {
        ...
        const importSignatures = new Map();
        const importList = [];
        for (const node of ast.body) {
            if (node.type === 'ImportDecl') {
                const imp = node;
                const sigs = (0, mmi_1.readMMIExports)(imp.path, path.dirname(filePath));
                const funcs = new Map();
                for (const e of sigs)
                    funcs.set(e.name, { params: e.params, ret: e.ret });
        ...
        if (command === 'mmi') {
            const { exports, permissions, imports } = (0, mmi_1.collectModuleInfo)(ast);
            const header = { name: path.parse(filePath).name, version: '1.0.0', exports, permissions, imports };
            const buffer = (0, mmi_1.packMMI)(header, llvmIR);
        ...
        const client = net.createConnection({ port: DAEMON_PORT }, () => {
            ...
            if (command === 'run') {
                for (const lib of nativeLibs) {
                    client.write((0, protocol_1.encodeFrame)(protocol_1.Cmd.LOAD_NATIVE, lib));
                }
                for (const imp of importList) {
                    client.write((0, protocol_1.encodeFrame)(protocol_1.Cmd.BIND_MMI, `${imp.alias} ${imp.path}`));
                }
