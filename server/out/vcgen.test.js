...
(0, node_test_1.test)('vcgen: for loop with invariant produces obligations', () => {
    const obs = obligations(`
        int32 sum(int32 n) ensures result >= 0 {
            int32 total = 0;
            for (int32 i = 0; i < n; i = i + 1) invariant total >= 0 {
                total = total + i;
            }
            return total;
        }
    `);
    node_assert_1.default.ok(obs.some(o => o.id.includes('for')));
});
