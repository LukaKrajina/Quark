...
public:
    ...
    void render(struct nk_context *ctx) override
    {
        ...
        nk_layout_row_begin(ctx, NK_STATIC, 25, 7);
        ...
        if (nk_menu_begin_label(ctx, "File", NK_TEXT_LEFT, nk_vec2(120, 200)))
        {
            ...
            nk_menu_item_label(ctx, "Recent Files", NK_TEXT_LEFT);
        ...
        {
            nk_layout_row_dynamic(ctx, 20, 1);
            nk_menu_item_label(ctx, "Select All", NK_TEXT_LEFT);
            nk_menu_item_label(ctx, "Deselect All", NK_TEXT_LEFT);
            nk_menu_item_label(ctx, "Invert Selection", NK_TEXT_LEFT);
            nk_menu_end(ctx);
        }
        ...
        {
            nk_layout_row_dynamic(ctx, 20, 1);
            nk_menu_item_label(ctx, "Refresh", NK_TEXT_LEFT);
            nk_menu_item_label(ctx, "Toggle Left", NK_TEXT_LEFT);
            nk_menu_item_label(ctx, "Toggle Right", NK_TEXT_LEFT);
            nk_menu_end(ctx);
        }

        nk_layout_row_push(ctx, 75);
        if (nk_menu_begin_label(ctx, "Training", NK_TEXT_LEFT, nk_vec2(150, 120)))
        {
            nk_layout_row_dynamic(ctx, 20, 1);
            nk_menu_item_label(ctx, "Start Training", NK_TEXT_LEFT);
            nk_menu_item_label(ctx, "Stop Training", NK_TEXT_LEFT);
            nk_menu_item_label(ctx, "Configure...", NK_TEXT_LEFT);
            nk_menu_end(ctx);
        }

        nk_layout_row_push(ctx, 70);
        if (nk_menu_begin_label(ctx, "Network", NK_TEXT_LEFT, nk_vec2(150, 120)))
        {
            nk_layout_row_dynamic(ctx, 20, 1);
            nk_menu_item_label(ctx, "Local Mode (LM)", NK_TEXT_LEFT);
            nk_menu_item_label(ctx, "Shared Mode (SM)", NK_TEXT_LEFT);
            nk_menu_item_label(ctx, "Quantum Net (QNM)", NK_TEXT_LEFT);
            nk_menu_end(ctx);
        }

        nk_layout_row_push(ctx, 50);
        if (nk_menu_begin_label(ctx, "Help", NK_TEXT_LEFT, nk_vec2(200, 120)))
        {
            nk_layout_row_dynamic(ctx, 20, 1);
            nk_menu_item_label(ctx, "About QQNT", NK_TEXT_LEFT);
            nk_menu_item_label(ctx, "Documentation", NK_TEXT_LEFT);
            nk_menu_item_label(ctx, "Keyboard Shortcuts", NK_TEXT_LEFT);
            nk_menu_end(ctx);
        }
...
public:
    ...
    void render(struct nk_context *ctx) override
    {
        ...
        nk_label(ctx, "Drive / Directory", NK_TEXT_LEFT);
...
public:
    ...
    {
        nk_layout_row_dynamic(ctx, 20, 1);
        nk_label(ctx, "Training Data", NK_TEXT_LEFT);

        nk_layout_row_dynamic(ctx, 20, 1);
        char path_buf[512];
        snprintf(path_buf, sizeof(path_buf), "Path: %s", tmp_path.c_str());
        nk_label(ctx, path_buf, NK_TEXT_LEFT);
...
public:
    ...
    {
        nk_layout_row_dynamic(ctx, 20, 1);
        nk_label(ctx, "Training Parameters", NK_TEXT_LEFT);
        nk_layout_row_begin(ctx, NK_STATIC, 25, 2);
        ...
        nk_layout_row_begin(ctx, NK_STATIC, 25, 2);
        nk_layout_row_push(ctx, 90);
        nk_label(ctx, "Epochs:", NK_TEXT_LEFT);
        nk_layout_row_push(ctx, 140);
        ...
        nk_layout_row_begin(ctx, NK_STATIC, 25, 2);
        nk_layout_row_push(ctx, 90);
        nk_label(ctx, "Learn Rate:", NK_TEXT_LEFT);
        nk_layout_row_push(ctx, 140);
        ...
        nk_layout_row_begin(ctx, NK_STATIC, 25, 2);
        nk_layout_row_push(ctx, 90);
        nk_label(ctx, "Qubits:", NK_TEXT_LEFT);
        nk_layout_row_push(ctx, 140);
        ...
        if (nk_button_label(ctx, "Start Training"))
        {
...
public:
    ...
    void render(struct nk_context *ctx) override
    {
        ...
        if (nk_selectable_label(ctx, "LM (Local Mode)", NK_TEXT_CENTERED, &is_lm))
        {
            if (is_lm)
        ...
        if (nk_selectable_label(ctx, "SM (Shared Mode)", NK_TEXT_CENTERED, &is_sm))
        {
            if (is_sm)
        ...
        if (nk_selectable_label(ctx, "QNM (Quantum Network Mode)", NK_TEXT_CENTERED, &is_qnm))
        {
            if (is_qnm)
        ...
        if (current_mode != OperationMode::LOCAL_MODE)
        {
            ...
            nk_layout_row_push(ctx, 150);
            {
                ...
                char buf[64];
                snprintf(buf, sizeof(buf), "Active Nodes: %zu", network_nodes.size());
                nk_label(ctx, buf, NK_TEXT_LEFT);
            }
            nk_layout_row_push(ctx, 200);
            nk_label(ctx, network_scanning ? "Status: Scanning..." : "Status: Ready", NK_TEXT_LEFT);
        ...
        {
            nk_layout_row_dynamic(ctx, 20, 1);
            nk_label(ctx, "Local drive mode — read/write from local filesystem", NK_TEXT_LEFT);
        }
    }
