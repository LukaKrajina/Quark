...
void *qk_create_BellState()
{
    ...
    size_t q0 = next_available_qubit++;
    size_t q1 = next_available_qubit++;
    obj->hardware_ids = {q0, q1};

    if (ActiveBackend)
    {
        ActiveBackend->allocate_qubits(next_available_qubit);
...
void *qk_create_QuantumRegister(int size)
{
    ...
    {
        obj->hardware_ids.push_back(next_available_qubit++);
    }
    if (ActiveBackend)
    {
        ActiveBackend->allocate_qubits(next_available_qubit);
...
void qk_release_object(void *ptr)
{
    ...
    if (obj)
    {
        ...
        next_available_qubit -= obj->hardware_ids.size();
        delete obj;
    }
}
...
void *__quantum__rt__qubit_allocate()
{
    if (!ActiveBackend)
        return nullptr;
    size_t id = next_available_qubit++;
    ActiveBackend->allocate_qubits(next_available_qubit);
