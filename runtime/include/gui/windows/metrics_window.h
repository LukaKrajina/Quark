...
namespace qgui
{
    ...
    {
    public:
        const char *title() const override { return "Metrics"; }
        ...
        void render(nk_context *ctx, const StateSnapshot &snap, float dt) override
        {
                         ...
                         NK_WINDOW_BORDER | NK_WINDOW_TITLE | NK_WINDOW_MOVABLE | NK_WINDOW_SCALABLE))
            {
                ...
                char l1[128];
                snprintf(l1, sizeof(l1), "FPS: %.1f   frame: %.2f ms", fps, avg * 1000.0);
                nk_label(ctx, l1, NK_TEXT_LEFT);

                char l2[128];
                snprintf(l2, sizeof(l2), "Daemon: %s   generation: %llu",
                         connected_ ? "connected" : "OFFLINE",
                         (unsigned long long)snap.generation);
                nk_label(ctx, l2, NK_TEXT_LEFT);

                char l3[128];
                snprintf(l3, sizeof(l3), "Backend: %s", snap.backend_name.c_str());
                nk_label(ctx, l3, NK_TEXT_LEFT);

                char l4[128];
                snprintf(l4, sizeof(l4), "Qubits: %u   amplitudes: %zu",
                         snap.num_qubits, snap.amplitudes.size());
                nk_label(ctx, l4, NK_TEXT_LEFT);

                char l5[128];
                snprintf(l5, sizeof(l5), "Present mode: %s", present_mode_.c_str());
