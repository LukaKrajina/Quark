...
namespace qgui
{
    ...
    {
    public:
        const char *title() const override { return "State Vector"; }
        ...
        void render(nk_context *ctx, const StateSnapshot &snap, float dt) override
        {
                         ...
                         NK_WINDOW_BORDER | NK_WINDOW_TITLE | NK_WINDOW_MOVABLE | NK_WINDOW_SCALABLE))
            {
                ...
                if (n == 0)
                {
                    ...
                    nk_label(ctx, "Waiting for state data...", NK_TEXT_LEFT);
                    nk_end(ctx);
                    return;
                }
                ...
                nk_layout_row_dynamic(ctx, 20, 1);
                char header[128];
                snprintf(header, sizeof(header), "%u qubits, %zu basis states (gen %llu)",
