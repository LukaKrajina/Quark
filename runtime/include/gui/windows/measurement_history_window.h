...
namespace qgui
{
    ...
    {
    public:
        const char *title() const override { return "Measurement History"; }
        ...
        void render(nk_context *ctx, const StateSnapshot &snap, float dt) override
        {
                         ...
                         NK_WINDOW_BORDER | NK_WINDOW_TITLE | NK_WINDOW_MOVABLE | NK_WINDOW_SCALABLE))
            {
                ...
                if (m == 0)
                {
                    ...
                    nk_label(ctx, "No measurements yet", NK_TEXT_LEFT);
                    nk_end(ctx);
                    return;
                }
                ...
                nk_layout_row_dynamic(ctx, 20, 1);
                char stat[128];
                snprintf(stat, sizeof(stat), "%zu samples: |0> = %d, |1> = %d", m, c0, c1);
                ...
                nk_layout_row_dynamic(ctx, 20, 1);
                char hline[128];
                snprintf(hline, sizeof(hline), "Last %zu: |0> = %d, |1> = %d", hcnt, bins[0], bins[1]);
