...
namespace qgui
{
    ...
    void load_i18n_fonts(struct nk_font_atlas *atlas)
    {
        ...
        if (!cjk.empty())
        {
            ...
            struct nk_font *f = nk_font_atlas_add_from_file(atlas, cjk.c_str(), height, &cfg);
            if (!f)
            {
                // CJK 字体加载失败（如 .ttc 集合不被 stb_truetype 支持），回退默认字体，
                // 确保 default_font 非空、ctx->style.font 有效，避免 nk_begin 崩溃。
