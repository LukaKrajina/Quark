#pragma once
// ============================================================================
// IntervalAbstract.hpp — 区间抽象判定引擎（引擎②）
//
// 对应论文 2 的「保守判定 / 弃权」语义。当引擎①（确定性符号执行）的反例搜索
...
public:
    ...
    static Solver::Result check(const Obligation& ob) {
        ...
        if (conse == Tri::True) {
            r.verdict = Solver::Verdict::Unsat;   // 已证明
            return r;
        }
        if (conse == Tri::False) {
...
// 组合引擎：引擎①（符号执行）→ unknown 时引擎②（区间抽象）兜底
inline std::string verifyProtocolCombined(const std::string& protocolText) {
    ...
    for (const auto& ob : obs) {
        Solver::Result res = Solver::check(ob);        // 引擎①：符号执行
        if (res.verdict == Solver::Verdict::Unknown) {
            res = IntervalAbstract::check(ob);          // 引擎②：区间抽象兜底
        }
