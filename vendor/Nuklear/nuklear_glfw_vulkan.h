...
#ifdef NK_MEMSET
#define NK_MEMSET(ptr, c, size) memset(ptr, c, size)
#endif
...
NK_INTERN void nk_glfw3_create_semaphore(struct nk_glfw_device *dev) {
    ...
    uint32_t i;
    uint32_t count = dev->image_views_len;

    memset(&semaphore_info, 0, sizeof(VkSemaphoreCreateInfo));
    semaphore_info.sType = VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO;

    dev->render_completed =
        (VkSemaphore *)malloc(count * sizeof(VkSemaphore));
    dev->render_completed_len = count;
    dev->fences = (VkFence *)malloc(count * sizeof(VkFence));
    dev->fences_len = count;
    ...
    for (i = 0; i < count; i++) {
        result = vkCreateSemaphore(dev->logical_device, &semaphore_info, NULL,
                                   &dev->render_completed[i]);
        NK_ASSERT(result == VK_SUCCESS);

        result = vkCreateFence(dev->logical_device, &fence_info, NULL,
                               &dev->fences[i]);
        NK_ASSERT(result == VK_SUCCESS);
    }
}
...
NK_API void nk_glfw3_device_create(
    ...
    {
        uint32_t slot_count = dev->image_views_len;
        uint32_t i;

        dev->vertex_buffer = (VkBuffer *)malloc(slot_count * sizeof(VkBuffer));
        dev->vertex_memory =
            (VkDeviceMemory *)malloc(slot_count * sizeof(VkDeviceMemory));
        dev->mapped_vertex = (void **)malloc(slot_count * sizeof(void *));
        dev->index_buffer = (VkBuffer *)malloc(slot_count * sizeof(VkBuffer));
        dev->index_memory =
            (VkDeviceMemory *)malloc(slot_count * sizeof(VkDeviceMemory));
        dev->mapped_index = (void **)malloc(slot_count * sizeof(void *));

        for (i = 0; i < slot_count; i++) {
            nk_glfw3_create_buffer_and_memory(dev, &dev->vertex_buffer[i],
                                              VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                                              &dev->vertex_memory[i], max_vertex_buffer);
            nk_glfw3_create_buffer_and_memory(dev, &dev->index_buffer[i],
                                              VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
                                              &dev->index_memory[i], max_element_buffer);

            vkMapMemory(dev->logical_device, dev->vertex_memory[i], 0,
                        max_vertex_buffer, 0, &dev->mapped_vertex[i]);
            vkMapMemory(dev->logical_device, dev->index_memory[i], 0,
                        max_element_buffer, 0, &dev->mapped_index[i]);
        }

        nk_glfw3_create_buffer_and_memory(dev, &dev->uniform_buffer,
...
NK_API void nk_glfw3_device_destroy(void) {
    ...
    {
        uint32_t i;
        for (i = 0; i < dev->render_completed_len; i++)
            vkDestroySemaphore(dev->logical_device, dev->render_completed[i], NULL);
        free(dev->render_completed);
        dev->render_completed_len = 0;

        for (i = 0; i < dev->fences_len; i++)
            vkDestroyFence(dev->logical_device, dev->fences[i], NULL);
        free(dev->fences);
        dev->fences_len = 0;

        for (i = 0; i < dev->image_views_len; i++) {
            vkUnmapMemory(dev->logical_device, dev->vertex_memory[i]);
            vkUnmapMemory(dev->logical_device, dev->index_memory[i]);

            vkFreeMemory(dev->logical_device, dev->vertex_memory[i], NULL);
            vkFreeMemory(dev->logical_device, dev->index_memory[i], NULL);

            vkDestroyBuffer(dev->logical_device, dev->vertex_buffer[i], NULL);
            vkDestroyBuffer(dev->logical_device, dev->index_buffer[i], NULL);
        }
        free(dev->vertex_buffer);
        free(dev->vertex_memory);
        free(dev->mapped_vertex);
        free(dev->index_buffer);
        free(dev->index_memory);
        free(dev->mapped_index);
...
NK_API void nk_glfw3_font_stash_end(VkQueue graphics_queue) {
    ...
    } else if (glfw.atlas.fonts) {
        nk_style_set_font(&glfw.ctx, &glfw.atlas.fonts->handle);
    }
}
...
VkSemaphore nk_glfw3_render(VkQueue graphics_queue, uint32_t buffer_index,
    ...
    /* Frames-in-flight: wait for the previous frame that used this swapchain
       image before overwriting its per-slot vertex/index buffers. */
    if (buffer_index < dev->fences_len) {
        vkWaitForFences(dev->logical_device, 1, &dev->fences[buffer_index],
                        VK_TRUE, UINT64_MAX);
        vkResetFences(dev->logical_device, 1, &dev->fences[buffer_index]);
    }
                            ...
                            &dev->uniform_descriptor_set, 0, NULL);
    {
         ...
         */
        {
            ...
            nk_buffer_init_fixed(&vbuf, dev->mapped_vertex[buffer_index],
                                 (size_t)dev->max_vertex_buffer);
            nk_buffer_init_fixed(&ebuf, dev->mapped_index[buffer_index],
        ...
        vkCmdBindVertexBuffers(command_buffer, 0, 1, &dev->vertex_buffer[buffer_index],
                               &doffset);
        vkCmdBindIndexBuffer(command_buffer, dev->index_buffer[buffer_index], 0,
    ...
    submit_info.pSignalSemaphores = &dev->render_completed[buffer_index];

    result = vkQueueSubmit(graphics_queue, 1, &submit_info, dev->fences[buffer_index]);
    NK_ASSERT(result == VK_SUCCESS);

    return dev->render_completed[buffer_index];
}
