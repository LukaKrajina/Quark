...
namespace quarkrsp::qcdrc
{
    ...
    public:
        Teleop()
        {
            std::cout << "[quarkRSP.qcdrc] QCDRC real-time teleop online.\n";
        }
        ...
        void map_motion(const Skeleton &skel)
        {
            ...
            for (const auto &j : skel.joints)
            {
                ...
                RobotJointTarget t;
                t.name = j.name;
                t.angle_rad = std::atan2(j.z, std::sqrt(j.x * j.x + j.y * j.y));
                joint_targets_.push_back(t);
            }
        }

        // IK 求解（骨架：直接返回目标作为关节角占位）
...
namespace quarkrsp::qcdrc
{
    ...
    public:
        ...
        // 动作映射：人体骨架 → 机器人关节目标
        // 从人体骨架 3D 关节位置用向量几何提取真实关节角：肩抬升角、肘弯曲角、腕倾角，对应义肢
