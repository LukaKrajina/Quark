...
namespace quarkrsp::qcdrc
{
    ...
    public:
        BehaviorCloner()
        {
            std::cout << "[quarkRSP.qcdrc] Behavioral cloning via quantum estimation ready.\n";
        }
        ...
        void train(int epochs)
        {
            if (dataset_.empty())
                return;
            std::cout << "[quarkRSP.qcdrc] Training policy from " << dataset_.size()
                      << " demos (" << epochs << " epochs)...\n";
            ...
            if (backend_ && !dataset_.front().observation.empty())
            {
                ...
                std::cout << "[quarkRSP.qcdrc] Quantum kernel self-overlap (calibration) = "
                          << self << " (ideal 1.0).\n";
            }
            ...
            std::cout << "[quarkRSP.qcdrc] Policy distribution estimated (bandwidth="
...
namespace quarkrsp::qcdrc
{
    ...
    public:
        ...
        // 量子核方法估计策略分布：
        // 由演示观测估计核带宽；
        // 量子后端在线时，用 QM/QVM 测量量子核自重叠 k(x,x)（应≈1）做校准；
        // epochs 用于带宽精化（轻量收缩，聚焦近邻）。
        void train(int epochs)
        {
            if (dataset_.empty())
                return;
