...
namespace quarkrsp::qpc
{
    ...
    public:
            ...
            : use_real_qm_(use_real_qm), dt_(dt)
        {
            ...
            std::cout << "[quarkRSP.qpc] Physics kernel online ("
        ...
        RigidBody &body(size_t i) { return bodies_[i]; }
        const RigidBody &body(size_t i) const { return bodies_[i]; }
        size_t body_count() const { return bodies_.size(); }
        const Collider &collider(size_t i) const { return colliders_[i]; }
    ...
    private:
        ...
        void detect_collisions()
        {
            ...
            {
                size_t ia = colliders_[i].body_index;
                if (ia >= body_enabled_.size() || !body_enabled_[ia])
                    continue;
                for (size_t j = i + 1; j < colliders_.size(); ++j)
                {
                    size_t jb = colliders_[j].body_index;
                    if (jb >= body_enabled_.size() || !body_enabled_[jb])
                        continue;
                    ...
                    Contact c;
                    if (dispatch(a, colliders_[i], b, colliders_[j], c))
