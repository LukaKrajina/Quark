...
namespace quarkrsp::gui
{
    ...
    CodeEditor::CodeEditor(QWidget *parent) : QPlainTextEdit(parent)
    {
        ...
        setStyleSheet("QPlainTextEdit{background:#1e1e1e;color:#d4d4d4;"
                      "selection-background:#264f78;selection-color:#ffffff;}");
    ...
    void CodeEditor::highlightCurrentLine()
    {
        ...
        {
            QTextEdit::ExtraSelection sel;
            sel.format.setBackground(QColor("#3aa0ff"));
        ...
        if (bracket_pos >= 0)
        {
            ...
            if (match >= 0)
            {
                ...
                {
                    QTextEdit::ExtraSelection sel;
                    sel.format.setBackground(QColor("#5a4a1a"));
    ...
    void CodeEditor::lineNumberAreaPaintEvent(QPaintEvent *event)
    {
        ...
        while (block.isValid() && top <= event->rect().bottom())
        {
            if (block.isVisible() && bottom >= event->rect().top())
            {
                ...
                {
                    painter.fillRect(0, top, width, bottom - top, QColor("#2a2a2a"));
                }

                QString number = QString::number(block_number + 1);
                if (block_number == current_line)
                    painter.setPen(QColor("#c8c8c8"));   // 当前行号更亮
                else
                    painter.setPen(QColor("#6a6a6a"));   // 其余行号更暗
    ...
    {
        // 键名（"key" 后跟冒号）
        key_fmt_.setForeground(QColor("#9cdcfe"));
        key_fmt_.setFontWeight(QFont::Bold);

        // 字符串值
        string_fmt_.setForeground(QColor("#ce9178"));

        // 数字
        number_fmt_.setForeground(QColor("#b5cea8"));

        // 字面量 true/false/null
        literal_fmt_.setForeground(QColor("#569cd6"));
    ...
    {
        auto *lay = new QVBoxLayout(this);
        auto *g = new QGroupBox("物理引擎 Physics", this);
        auto *gl = new QVBoxLayout(g);

        gl->addWidget(new QLabel("重力 g (m/s²):", g));
        gravity_slider_ = new QSlider(Qt::Horizontal, g);
        gravity_slider_->setRange(0, 300);  // 0 ~ 30.0（×10）
        gl->addWidget(gravity_slider_);

        gl->addWidget(new QLabel("求解迭代次数:", g));
        solver_spin_ = new QSpinBox(g);
        solver_spin_->setRange(1, 32);
        gl->addWidget(solver_spin_);

        status_ = new QLabel("运行中", g);
        gl->addWidget(status_);

        auto *btn_row = new QHBoxLayout();
        auto *pause = new QPushButton("暂停", g);
        auto *step = new QPushButton("单步", g);
    ...
    {
        auto *lay = new QVBoxLayout(this);
        auto *g = new QGroupBox("强化学习 RL", this);
        auto *gl = new QVBoxLayout(g);
        ...
        reward_ = new QLabel("Reward: 0", g);
        episode_ = new QLabel("Episode: 0", g);
        status_ = new QLabel("空闲", g);
        ...
        auto *btn_row = new QHBoxLayout();
        auto *start = new QPushButton("开始训练", g);
        auto *stop = new QPushButton("停止", g);
        btn_row->addWidget(start);
        btn_row->addWidget(stop);
        gl->addLayout(btn_row);
    ...
    void RlPanel::refresh(SimulationHost &host)
    {
        ...
        reward_->setText(QString("Reward: %1").arg(host.rl_reward(), 0, 'f', 3));
        episode_->setText(QString("Episode: %1").arg(host.rl_episode()));
    ...
    {
        auto *lay = new QVBoxLayout(this);
        auto *g = new QGroupBox("量子后端 Quantum", this);
        auto *gl = new QVBoxLayout(g);
    ...
    void QuantumPanel::refresh(SimulationHost &host)
    {
        ...
        qubits_->setText(QString("量子比特数: %1").arg(host.quantum_num_qubits()));
    ...
    {
        auto *lay = new QVBoxLayout(this);
        auto *g = new QGroupBox("意识控制器 Consciousness", this);
        auto *gl = new QVBoxLayout(g);
    ...
    {
        auto *lay = new QVBoxLayout(this);
        auto *g = new QGroupBox("脑机桥 BrainBridge", this);
        auto *gl = new QVBoxLayout(g);
    ...
    {
        auto *lay = new QVBoxLayout(this);
        auto *g = new QGroupBox("机器人电路 Circuit", this);
        auto *gl = new QVBoxLayout(g);
    ...
    {
        auto *lay = new QVBoxLayout(this);
        auto *g = new QGroupBox("蓝图 Blueprint", this);
        auto *gl = new QVBoxLayout(g);
        list_ = new QListWidget(g);
        gl->addWidget(list_);
        lay->addWidget(g);
    ...
    {
        auto *lay = new QVBoxLayout(this);
        auto *g = new QGroupBox("场景图 Scene Graph", this);
        auto *gl = new QVBoxLayout(g);
        list_ = new QListWidget(g);
        gl->addWidget(list_);
        lay->addWidget(g);
    ...
    {
        auto *lay = new QVBoxLayout(this);
        auto *g = new QGroupBox("日志 Log", this);
        auto *gl = new QVBoxLayout(g);
    ...
    {
        auto *lay = new QVBoxLayout(this);
        auto *g = new QGroupBox("指标 Metrics", this);
        auto *gl = new QVBoxLayout(g);
        fps_ = new QLabel(g);
    ...
    {
        fps_->setText(QString("FPS: %1").arg(fps, 0, 'f', 1));
        frame_ms_->setText(QString("帧时间: %1 ms").arg(frame_ms, 0, 'f', 2));
        step_->setText(QString("仿真步数: %1").arg(host.sim_step()));
        norm_residual_->setText(QString("态矢量残差 |1-‖ψ‖|: %1").arg(host.quantum_normalization_residual(), 0, 'e', 2));
    }
    ...
    {
        auto *lay = new QVBoxLayout(this);
        auto *g = new QGroupBox("分形地形 Fractal Terrain", this);
        auto *gl = new QVBoxLayout(g);

        enabled_ = new QCheckBox("启用分形地形", g);
        gl->addWidget(enabled_);

        gl->addWidget(new QLabel("分辨率（每边采样点）:", g));
        resolution_ = new QSpinBox(g);
        resolution_->setRange(16, 512);
        resolution_->setSingleStep(16);
        gl->addWidget(resolution_);

        gl->addWidget(new QLabel("范围 extent（截面 [-e, e]²）:", g));
        ...
        gl->addWidget(new QLabel("高度系数 height_scale:", g));
        ...
        auto *apply = new QPushButton("应用并重新生成", g);
        gl->addWidget(apply);
    ...
    {
        auto *lay = new QVBoxLayout(this);
        auto *g = new QGroupBox("实体属性 Entity Details", this);
        auto *gl = new QVBoxLayout(g);

        name_label_ = new QLabel("未选中实体", g);
        ...
        gl->addWidget(new QLabel("位置 Position (m):", g));
        ...
        gl->addWidget(new QLabel("旋转 Rotation (°):", g));
        ...
        gl->addWidget(new QLabel("缩放 Scale:", g));
        ...
        delete_btn_ = new QPushButton("删除实体", g);
                                         ...
                                         double rx, double ry, double rz, double sx, double sy, double sz)
    {
        ...
        kind_label_->setText(QString("类型: %1").arg(QString::fromStdString(kind)));
        mass_label_->setText(QString("质量: %1 kg").arg(mass, 0, 'f', 2));
        collider_label_->setText(QString("碰撞体: %1").arg(QString::fromStdString(collider)));
    ...
    {
        auto *lay = new QVBoxLayout(this);
        auto *g = new QGroupBox("资产属性 Asset Details", this);
        auto *gl = new QVBoxLayout(g);

        name_label_ = new QLabel("未选中资产", g);
        ...
        auto *btn_row = new QHBoxLayout();
        save_btn_ = new QPushButton("保存", g);
        save_btn_->setEnabled(false);
        discard_btn_ = new QPushButton("放弃修改", g);
        discard_btn_->setEnabled(false);
        undo_btn_ = new QPushButton("撤销", g);
        undo_btn_->setEnabled(false);
        redo_btn_ = new QPushButton("重做", g);
        ...
        undo_status_ = new QLabel("撤销栈: 0  重做栈: 0", g);
        undo_status_->setStyleSheet("color:#8a93a6;");
        gl->addWidget(undo_status_);

        auto *opt_row = new QHBoxLayout();
        tab_spaces_check_ = new QCheckBox("Tab 缩进(空格)", g);
        tab_spaces_check_->setChecked(true);
        wrap_check_ = new QCheckBox("自动换行", g);
    ...
    void AssetDetailsPanel::show_asset(const QString &path, const QString &type_name, int status)
    {
        ...
        type_label_->setText(QString("类型: %1").arg(type_name));
        path_label_->setText(QString("路径: %1").arg(fi.absoluteFilePath()));
        size_label_->setText(QString("大小: %1 字节").arg(fi.size()));
        size_label_->setText(QString("大小: %1 字节   修改时间: %2")
            .arg(fi.size())
            .arg(fi.lastModified().toString("yyyy-MM-dd hh:mm:ss")));

        QString status_text;
        if (status & AssetStatus::Dirty)      status_text += "*已修改 ";
        if (status & AssetStatus::CheckedOut) status_text += "✓已检出 ";
        if (status & AssetStatus::Added)      status_text += "+新增 ";
        if (status_text.isEmpty())            status_text = "干净";
        ...
        if (editable) {
            ...
            content_edit_->setReadOnly(true);
            content_edit_->setPlainText("（二进制资产，不支持文本编辑）");
        }
        update_buttons();
    ...
    void AssetDetailsPanel::load_content()
    {
        ...
        if (!f.open(QIODevice::ReadOnly | QIODevice::Text)) {
            content_edit_->setPlainText("（无法读取文件）");
            content_edit_->setReadOnly(true);
            return;
        }
    ...
    void AssetDetailsPanel::update_undo_status()
    {
        ...
        undo_status_->setText(QString("撤销栈: %1  重做栈: %2").arg(undo_steps).arg(redo_steps));
