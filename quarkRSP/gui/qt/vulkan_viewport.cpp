...
namespace quarkrsp::gui
{
    ...
    void ViewportRenderer::releaseResources()
    {
        ...
        if (desc_set_ && desc_pool_)
            vkFreeDescriptorSets(dev, desc_pool_, 1, &desc_set_);
        if (desc_pool_)
        ...
        if (sky_desc_set_ && sky_desc_pool_)
            vkFreeDescriptorSets(dev, sky_desc_pool_, 1, &sky_desc_set_);
        if (sky_desc_pool_)
        ...
        if (line_desc_set_ && line_desc_pool_)
            vkFreeDescriptorSets(dev, line_desc_pool_, 1, &line_desc_set_);
        if (line_desc_pool_)
    ...
    void ViewportRenderer::upload_meshes()
    {
        ...
        for (size_t i = 0; i < meshes_.size(); ++i)
        {
            ...
            create_buffer(cpu.vertices.size() * sizeof(render::Vertex),
    ...
    void ViewportRenderer::create_pipeline()
    {
        ...
        binding.binding = 0;
        binding.stride = sizeof(render::Vertex);
        binding.inputRate = VK_VERTEX_INPUT_RATE_VERTEX;

        std::vector<VkVertexInputAttributeDescription> attrs(4);
        attrs[0] = {0, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(render::Vertex, position)};
        attrs[1] = {1, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(render::Vertex, normal)};
        attrs[2] = {2, 0, VK_FORMAT_R32G32_SFLOAT, offsetof(render::Vertex, u)};
        attrs[3] = {3, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(render::Vertex, r)};
    ...
    void ViewportRenderer::startNextFrame()
    {
        ...
        // 相机
        QVector3D center(0.0f, 1.0f, 0.0f);
        ...
        QMatrix4x4 proj;
        proj.perspective(60.0f, float(sz.width()) / float(sz.height()), 0.1f, 100.0f);
        QMatrix4x4 view;
        ...
        for (const auto &inst : state_.instances)
        {
            ...
            Ubo ubo{};
            std::memcpy(ubo.mvp, mvp.constData(), sizeof(float) * 16);
            std::memcpy(ubo.model, model.constData(), sizeof(float) * 16);
            ubo.light_dir[0] = light_dir[0];
            ubo.light_dir[1] = light_dir[1];
            ubo.light_dir[2] = light_dir[2];
            ubo.light_intensity = 1.2f;
