...
namespace quarkrsp::gui
{
    ...
    QImage AsyncThumbnailLoader::thumbnail(const QString &path, AssetType type)
    {
        ...
        // 并发上限：在途任务已满则不再投递，等 worker 腾出空位后再来
    ...
    void ContentBrowser::build_ui()
    {
        ...
        path_label_->setStyleSheet("color:#8a93a6;");
        ...
        cancel_btn_->setToolTip("取消缩略图渲染");
        cancel_btn_->setVisible(false);
        filter_ = new QLineEdit();
        filter_->setPlaceholderText("过滤资产…");
    ...
    void ContentBrowser::update_progress()
    {
        ...
        if (total > 0 && done < total)
        {
            ...
            progress_bar_->setFormat(QString("缩略图 %1 / %2 (%3%)").arg(done).arg(total).arg(pct));
    ...
    void ContentBrowser::show_context_menu(const QPoint &pos)
    {
        ...
        QAction *open_act = menu.addAction("打开", this, [this]()
                                           {
        ...
        // ── 新建子菜单 ──────────────────────────────
        QMenu *new_menu = menu.addMenu("新建");
        new_menu->addAction("新建文件夹", this, [this]
                            { create_asset(AssetType::Folder); });
        new_menu->addAction("Qk 脚本 (.qk)", this, [this]
                            { create_asset(AssetType::Script); });
        new_menu->addAction("场景 (.qscene)", this, [this]
                            { create_asset(AssetType::Scene); });
        new_menu->addAction("蓝图 (.qbp)", this, [this]
                            { create_asset(AssetType::Blueprint); });
        new_menu->addAction("材质 (.qmat)", this, [this]
                            { create_asset(AssetType::Material); });
        new_menu->addAction("机器人 (.qrobot)", this, [this]
                            { create_asset(AssetType::Robot); });
        new_menu->addAction("文本 (.txt)", this, [this]
                            { create_asset(AssetType::Unknown); });

        menu.addAction("粘贴", this, &ContentBrowser::paste_from_clipboard);
        menu.addSeparator();

        // ── 查看子菜单 ──────────────────────────────
        QMenu *view_menu = menu.addMenu("查看");
        ...
        add_view("超大图标", ViewMode::ExtraLargeIcons);
        add_view("大图标", ViewMode::LargeIcons);
        add_view("列表", ViewMode::List);
        add_view("详细信息", ViewMode::Details);
    ...
    {
        bool ok = false;
        QString name = QInputDialog::getText(this, "新建资产", "名称：", QLineEdit::Normal, "", &ok);
