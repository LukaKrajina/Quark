...
namespace quarkrsp::gui
{
    ...
    {
        setWindowTitle(QString("qk 脚本编辑器 — %1").arg(QFileInfo(path).fileName()));
        resize(900, 680);
        ...
        auto *run_btn = new QPushButton("▶ 运行");
        auto *save_btn = new QPushButton("保存 (Ctrl+S)");
        auto *close_btn = new QPushButton("关闭");
        ...
        output_->setPlaceholderText("运行输出将显示在这里…");
        output_->setStyleSheet("QPlainTextEdit{background:#1e1e1e;color:#c8ccd4;}");
    ...
    void ScriptEditorWindow::run_script()
    {
        ...
        if (!output_)
            return;
        output_->clear();
        output_->appendPlainText(QString("[运行] %1").arg(path_));
        ...
        // ── 运行前检查 Quark Daemon（50052 端口）是否在线 ────────────
        {
            QTcpSocket probe;
            probe.connectToHost("127.0.0.1", 50052);
            if (!probe.waitForConnected(800))
            {
                output_->appendPlainText("[错误] Quark Daemon 未运行（端口 50052）。");
                output_->appendPlainText("       请先启动: runtime --daemon");
                output_->appendPlainText("       然后再点击运行。");
                return;
            }
        ...
        if (qk.isEmpty())
        {
            output_->appendPlainText("[错误] 未找到 qk.cmd 解释器");
            return;
        }
        ...
        connect(process_, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished),
                ...
                {
        output_->appendPlainText("------------------------------------------------");
        output_->appendPlainText(QString("[完成] 退出码 %1").arg(code)); });
